...
#
# In this utility function, we analyze the **three most significant POIs over 12 weeks** by default.
# A POI corresponds to three main locations:
#   - **Home** (22:00 to 06:00)
#   - **Workplace** (09:00 to 16:00, excluding lunch hours)
...
work_start, work_end = 9, 16  # From 09:00 to 16:00 (excluding lunch break)
lunch_hours = [12, 13]  # Lunch break hours
...
def main(original_file, anonymized_file, parameters=None):
    ...
    num_POIs = parameters.get("num_POIs", 3)
        ...
        for line_ori, line_ano in zip(original_reader, anonymized_reader):
            ...
            if timestamp.weekday() < 5:  # Weekday
                if timestamp.time() > datetime.time(night_start, 0) or timestamp.time() < datetime.time(night_end, 0):
                    original_POIs[user_id]['night'][location] += compute_time_difference(user_id, timestamp, last_date_original)
                elif work_start <= timestamp.time().hour < work_end and timestamp.time().hour not in lunch_hours:
                    original_POIs[user_id]['work'][location] += compute_time_difference(user_id, timestamp, last_date_original)
            ...
            if line_ano[0] != "DEL":
                ...
                if timestamp.weekday() < 5:  # Weekday
                    if timestamp.time() > datetime.time(night_start, 0) or timestamp.time() < datetime.time(night_end, 0):
                        anonymized_POIs[user_id]['night'][location] += compute_time_difference(user_id, timestamp, last_date_anonymized)
                    elif work_start <= timestamp.time().hour < work_end and timestamp.time().hour not in lunch_hours:
                        anonymized_POIs[user_id]['work'][location] += compute_time_difference(user_id, timestamp, last_date_anonymized)

    total_pois = sum(len(original_POIs[user_id][poi_type]) for user_id in original_POIs for poi_type in original_POIs[user_id])
    score = 0

    for user_id in original_POIs:
        for poi_type in original_POIs[user_id]:
            for location in original_POIs[user_id][poi_type]:
                original_time = original_POIs[user_id][poi_type][location].total_seconds()
                anonymized_time = anonymized_POIs[user_id][poi_type][location].total_seconds()
